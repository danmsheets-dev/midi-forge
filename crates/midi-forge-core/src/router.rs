...
/// One thru connection: source port → destination port, with a filter.
...
impl Router {
    ...
    /// Apply matching filters and emit one event per destination port.
    pub fn route(&self, incoming: &MidiEvent) -> Vec<MidiEvent> {
        ...
        for link in &self.links {
            ...
            if let Some(packet) = link.filter.apply(&incoming.packet) {
